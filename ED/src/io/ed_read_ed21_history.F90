...
#if defined(COUPLED)
         !---------------------------------------------------------------------------------!
         !   Allocate the vector of sites in the polygon. For coupled runs, we only use    !
         !   these land(dry) sites.                                                        !
         !---------------------------------------------------------------------------------!
...
#else
         !---------------------------------------------------------------------------------!
         !   Allocate the vector of sites in the polygon. For non-coupled runs, we use     !
         !   all sites (both land and lake sites).                                         !
         !---------------------------------------------------------------------------------!
         call allocate_polygontype(cpoly,pysi_n(py_index))
         cpoly%islakesite(:) = islakesite(:)

         is = 0
         siteloop: do isi=1,pysi_n(py_index)

            if (islakesite(isi) == 0 .or. islakesite(isi) == 1) then
               ...
               if (sipa_n(isi) > 0) then

                  !----- Fill 1D polygon (site unique) level variables. -------------------!
                  call allocate_sitetype(csite,sipa_n(isi))
                  ...
                  chnkoffs(1) = int(sipa_id(isi) - 1,8)
      ...
      rstfileloop: do nf=1, nhisto
         ...
         polyloop: do ipy = 1,cgrid%npolygons
            ...
            siteloop2: do isi = 1,cpoly%nsites
...
#else
                  if (islakesite(isi_try) == cpoly%islakesite(isi)) then
#endif
                     is_try    = is_try + 1
               ...
               !---------------------------------------------------------------------------!
               ! Some sanity check here. is_best should be a positive number unless csite  !
               ! is a lakesite and there is no lakesite in the hdf5 file.                  !
               !---------------------------------------------------------------------------!
#if defined(COUPLED)
#else
               if (cpoly%islakesite(isi)==1 .and. is_best==-1) then
                  call init_ed_lakesite(cpoly,csite,isi)
                  cycle siteloop2
               elseif (is_best==-1) then
                  !----- Stop the run. -------------------------------------!
                   write (unit=*,fmt='(a)') 'There is no a single land site in history file'
                   call fatal_error('No land site in history file'                         &
