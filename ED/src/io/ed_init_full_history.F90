...
#else
   use ed_max_dims,only : n_pft,n_dbh,n_dist_types
   use mem_polygons,only : include_lakesite
#endif
...
#if defined(COUPLED)
   do i = 1, cpoly%nsites
      cpoly%islakesite(i) = 0
   end do
#else
   if ( include_lakesite > 0 ) then
      call hdf_getslab_i(cpoly%islakesite,'ISLAKESITE ',dsetrank,iparallel,.true.,foundvar)
   else
      do i = 1, cpoly%nsites
         cpoly%islakesite(i) = 0
      end do
...
!        dsetrank,iparallel,.true.,foundvar) 
 ...
 subroutine fill_history_site(csite,sipa_index,npatches_global)
  ...
  interface
   ...
   call hdf_getslab_r(csite%avg_monthly_temp,'AVG_MONTHLY_TEMP ',dsetrank,iparallel,.true.,foundvar)  
   ...
   call hdf_getslab_r(csite%avg_monthly_topwaterdiff,'AVG_MONTHLY_TOPWATERDIFF ',dsetrank,iparallel,.true.,foundvar)  
