...
subroutine count_pft_xml_config(filename,maxpft)
  ...
  print*,""
  print*,"============================================"
  print*,"Counting pfts in xml: ",trim(filename)
...
!  use ed_data
  ...
  logical,save :: mod_rho = .false.
  logical,save :: mod_treefall = .false.
  logical,save :: mod_mort3 = .false.
  logical,save :: mod_ltr = .false.
  logical,save :: mod_rtr = .false. 
  logical,save :: mod_sla = .false.

  ! mort3 is dependant on rho and treefall
  mod_rho = .false.       
  mod_treefall = .false.
  mod_mort3 = .false.
  
  ! SLA is dependant on leaf_turnover_rate
  mod_sla = .false.
  mod_ltr = .false.

  ! Root turnover rate is dependant on leaf_turnover_rate
  mod_rtr = .false.
  ...
  if(ntag .ge. 1) then
     do i=1,ntag
        call libxml2f90__ll_selecttag('DOWN','misc',i)
         ...
         call getConfigREAL  ('gamm','misc',i,rval,texist)
         if(texist) gamm = real(rval)
         call getConfigREAL  ('gamh','misc',i,rval,texist)
         if(texist) gamh = real(rval)

         call getConfigREAL  ('tprandtl','misc',i,rval,texist)
         if(texist) tprandtl = real(rval)

         call getConfigREAL  ('ribmax','misc',i,rval,texist)
         if(texist) ribmax = real(rval)

         call getConfigREAL  ('leaf_maxwhc','misc',i,rval,texist)
         if(texist) leaf_maxwhc = real(rval)

         call getConfigREAL  ('leaf_drywhc','misc',i,rval,texist)
         if(texist) leaf_drywhc = real(rval)

         call getConfigREAL  ('ubmin','misc',i,rval,texist)
         if(texist) ubmin = real(rval)

         call getConfigREAL  ('ugbmin','misc',i,rval,texist)
         if(texist) ugbmin = real(rval)

         call getConfigREAL  ('ustmin','misc',i,rval,texist)
         if(texist) ustmin = real(rval)
...
!              include_pft(myPFT) = 1  !! if a PFT is defined, assume it's meant to be included
           ...
           call getConfigREAL  ('leaf_emis','pft',i,rval,texist)
           if(texist) leaf_emis(myPFT) = dble(rval)
           call getConfigREAL  ('wood_emis','pft',i,rval,texist)
           if(texist) wood_emis(myPFT) = dble(rval)
           ...
           ! Check on combined transmittance and reflectance

           if  (leaf_reflect_vis(myPFT) < 0.01 .or. leaf_reflect_vis(myPFT) > 0.99) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                             &
                   'Invalid LEAF_REFLECT_VIS, it must be between 0.01 and 0.99.'      &
                   ,'Yours is set to',leaf_reflect_vis(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if

           if  (leaf_reflect_nir(myPFT) < 0.01 .or. leaf_reflect_nir(myPFT) > 0.99) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                             &
                   'Invalid LEAF_REFLECT_NIR, it must be between 0.01 and 0.99.'      &
                   ,'Yours is set to',leaf_reflect_nir(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if

           if  (leaf_trans_vis(myPFT) < 0.01 .or. leaf_trans_vis(myPFT) > 0.99) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                             &
                   'Invalid LEAF_TRANS_VIS, it must be between 0.01 and 0.99.'      &
                   ,'Yours is set to',leaf_trans_vis(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if

           if  (leaf_trans_nir(myPFT) < 0.01 .or. leaf_trans_nir(myPFT) > 0.99) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                             &
                   'Invalid LEAF_TRANS_NIR, it must be between 0.01 and 0.99.'      &
                   ,'Yours is set to',leaf_trans_nir(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if


           if  (leaf_reflect_vis(myPFT) + leaf_trans_vis(myPFT) > 0.99 ) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                             &
                   'Leaf visible transmittance+reflectance cannot exceed 0.99.'      &
                   ,'Yours is set to',leaf_reflect_vis(myPFT)+leaf_trans_vis(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if

           if  (leaf_reflect_nir(myPFT) + leaf_trans_nir(myPFT) > 0.99 ) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                             &
                   'Leaf NIR transmittance+reflectance cannot exceed 0.99.'      &
                   ,'Yours is set to',leaf_reflect_nir(myPFT)+leaf_trans_nir(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if
           

           if  (wood_reflect_vis(myPFT) + wood_trans_vis(myPFT) > 0.99 ) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                             &
                   'Wood visible transmittance+reflectance cannot exceed 0.99.'      &
                   ,'Yours is set to',wood_reflect_vis(myPFT)+wood_trans_vis(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if

           if  (wood_reflect_nir(myPFT) + wood_trans_nir(myPFT) > 0.99 ) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                             &
                   'Wood NIR transmittance+reflectance cannot exceed 0.99.'      &
                   ,'Yours is set to',wood_reflect_nir(myPFT)+wood_trans_nir(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if

           if  (orient_factor(myPFT) < -0.40 .or. orient_factor(myPFT) > 0.60) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                           &
                   'Invalid orient_factor, it must be between -0.40 and 0.60.'    &
                   ,'Yours is set to',orient_factor(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if

           if  (clumping_factor(myPFT) < 0.01 .or. clumping_factor(myPFT) > 1.00) then
              write (reason,fmt='(a,2x,a,1x,es12.5,a)')                           &
                   'Invalid clump_factor, it must be between 0.01 and 1.00.'      &
                   ,'Yours is set to',clumping_factor(myPFT),'...'
              call opspec_fatal(reason,'ed_xml_config')
           end if
           
           ! THESE VARIABLES WILL BREAK DEPENDANCIES IF MODIFIED
...
!! THIS BREAKS DEPENDANCIES IN ed_params.f90
...
!!$           if(texist) c2n_storage(myPFT) = real(rval)
           ...
           ! MODIFYING WOOD DENSITY IS A REAL BAD IDEA
           ! ALL SORTS OF STUFF IS DEPENDANT ON THIS ONE
           ! IF A USER REALLY WANTS TO USE IT, THEY SHOULD MODIFY THIS LINE
           ! RGK 2013-03
           call getConfigREAL  ('rho','pft',i,rval,texist)
           if(texist) then
              rho(myPFT) = real(rval)
              print*,"======================================================"
              print*," YOU ARE TRYING TO MODIFY WOOD DENSITY IN THE XML."
              print*," THIS IS A COMPLICATED PARAMETER.  THERE ARE MANY"
              print*," PARAMETERS THAT DEPEND ON THIS."
              print*," IF YOU *MUST CHANGE THIS PARAMETER, IDENTIFY THE"
              print*," VARIABLES IN ed_params.f90 DEPENDANT ON rho()"
              print*," AND ACCOUNT FOR THOSE DEPENDANCIES...."
              print*," OR.. CHANGE THE HARD-CODED PARAMETERS IN ed_params.f90"
              print*," OPTION 2 IS PROBABLY EASIER."
              print*," STOPPING"
              print*,"======================================================"
              stop
           end if
           ...
           call getConfigREAL  ('flushing_temp_min','pft',i,rval,texist)
           if(texist) flushing_temp_min(myPFT) = real(rval)
           call getConfigREAL  ('flushing_temp_max','pft',i,rval,texist)
           if(texist) flushing_temp_max(myPFT) = real(rval)
           call getConfigREAL  ('dl_min','pft',i,rval,texist)
           if(texist) dl_min(myPFT) = real(rval)
           call getConfigREAL  ('dl_max','pft',i,rval,texist)
           if(texist) dl_max(myPFT) = real(rval)
           call getConfigREAL  ('dry_period','pft',i,rval,texist)
           if(texist) dry_period(myPFT) = real(rval)
           call getConfigREAL  ('r_fract','pft',i,rval,texist)
           if(texist) r_fract(myPFT) = real(rval)
           ...
           if(texist) then
              leaf_turnover_rate(myPFT) = real(rval)
              mod_ltr = .true.
           end if

           call getConfigREAL  ('root_turnover_rate','pft',i,rval,texist)
           if(texist) then
              root_turnover_rate(myPFT) = real(rval)
              mod_rtr = .true.
           end if
...
!! REMOVED BECAUSE IT IS A DERIVED PARAMETER
!! USER SHOULD CHANGE leaf_reflect_vis and leaf_trans_vis INSTEAD
!!           call getConfigREAL  ('leaf_scatter_vis','pft',i,rval,texist)
!!           if(texist) leaf_scatter_vis(myPFT) = real(rval)

!! REMOVED BECAUSE THIS IS A DERIVED PARAMETER, SEE ed_params.f90
...
!        if(texist) c2n_stem = real(rval)
        ...
        if(texist) then
           print*,"======================================================"
           print*," YOU ARE TRYING TO MODIFY THE CARBON TO BIOMASS (C2B)."
           print*," THIS IS A COMPLICATED PARAMETER.  THERE ARE MANY"
           print*," PARAMETERS THAT DEPEND ON THIS."
           print*," IF YOU *MUST CHANGE THIS PARAMETER, IDENTIFY THE"
           print*," VARIABLES IN ed_params.f90 DEPENDANT ON C2B"
           print*," AND ACCOUNT FOR THOSE DEPENDANCIES...."
           print*," OR.. CHANGE THE HARD-CODED PARAMETERS IN ed_params.f90"
           print*," AND RECOPILE.  OPTION 2 IS PROBABLY EASIER."
           print*," STOPPING"
           print*,"======================================================"
           stop
        end if
...
!!  THESE ARE DERIVED VARIABLES, SEE ed_params.f90
...
!! THIS WILL BREAK DEPENANCIES  (rk4water_stab_thresh)
...
!! THESE DATABASES ARE CALLED WAYYYY BEFORE THE XML READ
...
!!         end if
         ...
         ! THIS STUFF IS ACTUALLY CALLED AFTE THE XML READ
  ...
  if(ntag .ge. 1) then
     do i=1,ntag
        ...
        if(texist) then
           treefall_disturbance_rate = real(rval)
           mod_treefall = .true.
        end if
  ...
  if(ntag .ge. 1) then
     do i=1,ntag
        ...
        if(texist) radint = real(rval)

        call getConfigREAL  ('radslp','phenology',i,rval,texist)
        if(texist) radslp = real(rval)
  ...
  if(ntag .ge. 1) then
     do i=1,ntag
        ...
        call getConfigREAL  ('klowco2','physiology',i,rval,texist)
        if(texist)  klowco2 = real(rval)

        call getConfigREAL  ('ddmort_const','physiology',i,rval,texist)
        if(texist)  ddmort_const = real(rval)
  ...
  if ( mod_ltr .and. (.not.mod_rtr)) then
     print*," ============================================================="
     print*," IF YOU SET LEAF_TURNOVER_RATE IN THE XML"
     print*," YOU MUST ALSO SET ROOT_TURNOVER_RATE TO ENSURE CONSISTENCY."
     print*," "
     print*," TRY: grep leaf_turnover_rate ed_params.f90 "
     print*," TO SEE HOW DEFAULT PROPERTIES ARE SET"
     print*," STOPPING RUN"
     print*," =============================================================="
     stop
  end if

  if ( mod_ltr .and. (.not.mod_sla)) then
     print*," ============================================================="
     print*," IF YOU SET LEAF_TURNOVER_RATE IN THE XML"
     print*," YOU MUST ALSO SET SLA (specific leaf area) TO ENSURE CONSISTENCY."
     print*," "
     print*," TRY: grep SLA ed_params.f90 "
     print*," TO SEE HOW DEFAULT PROPERTIES ARE SET"
     print*," STOPPING RUN"
     print*," =============================================================="
     stop
  ...
  ! CHECK CONSISTENCY OF MORTALITY, TREEFALL AND WOOD DENSITY PARAMETERS
  if( mod_treefall .and. (.not.mod_mort3))  then
     print*," ============================================================="
     print*," IF YOU SET TREEFALL DISTURBANCE RATE                      "
     print*," IN THE XML, YOU MUST ALSO SET MORT3 FOR CONSISTENCY."
     print*," "
     print*," TRY: grep mort3 ed_params.f90 "
     print*," TO SEE HOW DEFAULT PROPERTIES ARE SET"
     print*," STOPPING RUN"
     print*," =============================================================="
     stop
...
subroutine getConfigREAL(ctag,ptag,pwhich,val,texist)
  ...
  if(j .gt. 0) then
     ...
     call libxml2f90__existid(ctag,texist)
     if(texist) then
        call libxml2f90__ll_getr8(ctag,1,val)
        if(report)then
           write(*,'(a,a,a,a,a,F12.6)') 'Reading ',ptag," ",ctag,"= ",val
        end if
     end if
     call libxml2f90__ll_selecttag('UP',ptag,pwhich)
  endif
...
!!produce a record of parameters used in a run
  ...
  do i=1,n_pft
     if(include_pft(i)) then !! loop over included PFT's
        ...
        call putConfigREAL("flushing_temp_min",flushing_temp_min(i))
        call putConfigREAL("flushing_temp_max",flushing_temp_max(i))
        call putConfigREAL("dl_min",dl_min(i))
        call putConfigREAL("dl_max",dl_max(i))
        call putConfigREAL("dry_period",dry_period(i))
        call putConfigREAL("r_fract",r_fract(i))
        ...
        ! NOTE!! SOME OF THE DEFAULT ROOT_TURNOVER_RATE PARAMETERS
        ! ARE BASED OFF OF LEAF_TURNOVER_RATE.  END USERS
        ! SHOULD CONSIDER CHANGING ROOT TURNOVER RATES IF LEAF_TURNOVER_RATES
        ! ARE MODIFIED IN THE XML FOR CONSISTENCY
