...
subroutine copy_nl(copy_type)
   ...
   use mem_polygons         , only : n_poi                     & ! intent(out)
                                   ...
                                   , grid_type                 & ! intent(out)
                                   , edres                     & ! intent(out)
                                   , include_lakesite          & ! intent(out)
                                   , waterfrac                 & ! intent(out)
                                   , maxsite                   & ! intent(out)
                                   , maxpatch                  & ! intent(out)
                                   , maxcohort                 ! ! intent(out)
#endif
   ...
   case ('ALL_CASES')
      ...
      soilwater_db              = nl%soilwater_db
      n_soilwater_points        = nl%n_soilwater_points
      if (n_soilwater_points>0) then
          soilwater_years(1:n_soilwater_points)  = nl%soilwater_years(1:n_soilwater_points)
          soilwater_months(1:n_soilwater_points) = nl%soilwater_months(1:n_soilwater_points)
          soilwater_dates(1:n_soilwater_points)  = nl%soilwater_dates(1:n_soilwater_points)
      else
          soilwater_years(1:2)                   = nl%soilwater_years(1:2)
          soilwater_months(1:2)                  = nl%soilwater_months(1:2)
          soilwater_dates(1:2)                   = nl%soilwater_dates(1:2)
      end if
      ...
      phenformat                = nl%phenformat
      phenpath                  = nl%phenpath
#if defined(COUPLED)
#else
      include_lakesite          = nl%include_lakesite
      waterfrac                 = nl%waterfrac
#endif
      maxsite                   = nl%maxsite
