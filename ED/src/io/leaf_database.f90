...
!==========================================================================================!
!==========================================================================================!
subroutine read_soilinfo(ofn,nsite,nlandsea,lat,lon,classout)
   ...
   allocate(idims0(nlandsea,2))
   !  Determine the indices of the grid that each polygon corresponds to.
   do ilandsea=1,nlandsea
       min_dist = huge (1.)
       if ( lat(1,ilandsea) < latmin .or. lat(1,ilandsea) > latmax .or.                 &
            lon(1,ilandsea) < lonmin .or. lon(1,ilandsea) > lonmax ) then
            write(unit=*,fmt='(2a)')                                                    &
            ...
            call fatal_error ('One polygon is out of the range of your provided soil data...'  &
                       ,'read_soilinfo','leaf_database.f90')
       end if
       
       lonloop: do ilon = 1,nlon
          latloop: do ilat = 1,nlat
             this_dist = dist_gc(lon(1,ilandsea), lon2d(ilon,ilat) ,lat(1,ilandsea), lat2d(ilon,ilat) )

             if(this_dist < min_dist) then
                idims0(ilandsea,1) = ilon
                idims0(ilandsea,2) = ilat
                min_dist  = this_dist
             end if
   ...
   allocate(tmp_data(idims(1),idims(2)))
   call shdf5_irec_f(ndims, idims, 'nslcon', rvara = tmp_data )
   do ilandsea=1,nlandsea
      classout(:,ilandsea)=tmp_data(idims0(ilandsea,1),idims0(ilandsea,2))
   end do
...
!==========================================================================================!
!==========================================================================================!
subroutine read_mask(ofn,nsite,nlandsea,lat,lon,classout)
   ...
   real           , dimension(    3,nlandsea), intent(in)  :: lat
   real           , dimension(    3,nlandsea), intent(in)  :: lon
   real           , dimension(nsite,nlandsea), intent(out) :: classout
   !----- Local variables. ----------------------------------------------------------------!
   integer                                                 :: ndims
   integer                                                 :: d
   integer        , dimension(2)                           :: idims
   ...
   allocate(tmp_data(idims(1),idims(2)))
   call shdf5_irec_f(ndims, idims, 'mask', rvara = tmp_data )
   do ilandsea=1,nlandsea
      if (tmp_data(idims0(ilandsea,1),idims0(ilandsea,2)) < 0.5) then
         classout(:,ilandsea)=0.0
      else
         classout(:,ilandsea)=1.0
      end if
   end do
