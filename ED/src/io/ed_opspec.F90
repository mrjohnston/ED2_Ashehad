...
subroutine ed_opspec_misc
...
#if defined(COUPLED)
#else
   if (include_lakesite < 0 .or. include_lakesite > 2) then
      write (reason,fmt='(a,1x,a,1x,i5,a)')                                                &
         'Invalid INCLUDE_LAKESITE, it must be between 0 and 2.'                           &
        ,'Yours is set to ',include_lakesite,'...'
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if

   if (include_lakesite == 2) then
      if (n_poi /= 1 .or. n_ed_region > 0) then
         write (reason,fmt='(a,1x,a)')                                                     &
            'Invalid INCLUDE_LAKESITE, it can be set to 2 only when n_poi=1 and'           &
           ,'n_ed_region=0. ...'
         call opspec_fatal(reason,'opspec_misc')
         ifaterr = ifaterr +1
      end if

      if (waterfrac < 0 .or. waterfrac > 1.0) then
         write (reason,fmt='(a,1x,a,1x,i5,a)')                                             &
            'Invalid WATERFRAC, it must be between 0 and 1.'                               &
           ,'Yours is set to ',waterfrac,'...'
         call opspec_fatal(reason,'opspec_misc')
         ifaterr = ifaterr +1
      end if
   end if

   if (include_lakesite > 0 .and. (ied_init_mode == 1 .or. ied_init_mode == 2 .or.         &
      ied_init_mode == 3 .or. ied_init_mode == 6) ) then
      
      write (reason,fmt='(4(a,1x),i5,a)')                                                  &
         'Wrong combination of INCLUDE_LAKESITE and IED_INIT_MODE! '                       &
         ,'If you intend to simulate a lake site (i.e. INCLUDE_LAKESITE>0),'               &
         ,'your IED_INIT_MODE should be set to the options except 1, 2, 3 and 6.'          &
         ,' Yours is set to',ied_init_mode       &
         ,'...'
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if

   if (include_lakesite > 0 .and. ied_init_mode == 4) then
      write (unit=*,fmt='(a)') ' '
      write (unit=*,fmt='(a)') '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'   
      write (unit=*,fmt='(a)') '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'   
      write (unit=*,fmt='(a)') '!!!  WARNING! WARNING! WARNING! WARNING! WARNING!   !!!'   
      write (unit=*,fmt='(a)') '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'   
      write (unit=*,fmt='(a)') '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'   
      write (unit=*,fmt='(2(a,1x,i2),1x,a,1x,a)') ' -> You set INCLUDE_LAKESITE to'        &
                               , include_lakesite, ' and IED_INIT_MODE to', ied_init_mode  &
                               , '. Your intention to add a lakesite may fail if there is' &
                               , 'no lakesite(s) in the history file(s). IED_INIT_MODE='   &
                               , '5 might be a better choice.'
      write (unit=*,fmt='(a)') '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'   
      write (unit=*,fmt='(a)') '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'   
      write (unit=*,fmt='(a)') ' '                                                         
   ...
   if (maxsite < 1 .or. maxsite > ed_nstyp) then
   ...
   if (n_soilwater_points <-1 .and. n_soilwater_points>60 ) then
      write (reason,fmt='(a,1x,i4,1x,a,1x,i4,a)')                                       &
             'Invalid N_SOILWATER_POINTS, it must be between -1 and 60. Yours is set to'     &
             ,n_soilwater_points,'for grid',ifm,'...'
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if
...
do ifm=1,ngrids
   ...
   if (phenformat < 0 .or. phenformat > 1) then
      write (reason,fmt='(a,1x,i4,a)')                                                     &
                    'Invalid PHENFORMAT, it must be between 0 and 1. Yours is set to'      &
                    ,phenformat,'...'
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if

   if (repro_scheme < 0 .or. repro_scheme > 4) then
      write (reason,fmt='(a,1x,i4,a)')                                                     &
                    'Invalid REPRO_SCHEME, it must be between  0 and 4. Yours is set to'   &
                    ,repro_scheme,'...'
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if
