module fuse_fiss_utils
         ...
         do ico = 1,size(split_mask)
            ...
            if (split_mask(ico)) then
               ...
               if (is_grass(cpatch%pft(ico)) .and. igrass==1) then 
                   ...
                   cpatch%hite (ico)  = max( hgt_min(cpatch%pft(ico)),                     &
                                            bl2h(cpatch%bleaf(ico), cpatch%pft(ico)) )
                   cpatch%dbh  (ico)  = h2dbh(cpatch%hite(ico),cpatch%pft(ico))

                   cpatch%bleaf(inew) = cpatch%bleaf(inew) * (1.+epsilon)
                   cpatch%hite (inew) = max( hgt_min(cpatch%pft(inew)),                     &
                                            bl2h(cpatch%bleaf(inew), cpatch%pft(inew)) )
                   cpatch%dbh  (inew) = h2dbh(cpatch%hite(inew),cpatch%pft(inew))
               else
                   !-- use bdead for trees
   ...
   subroutine fuse_2_cohorts(cpatch,donc,recc, newn,green_leaf_factor,can_prss,can_shv,lsl)
      ...
      if (is_grass(cpatch%pft(donc)) .and. igrass==1) then
          ...
          cpatch%hite (recc) = max( hgt_min(cpatch%pft(recc)),                             &
                                            bl2h(cpatch%bleaf(recc), cpatch%pft(recc)) )
          cpatch%dbh  (recc) = h2dbh(cpatch%hite(recc),cpatch%pft(recc))
      else
          !--use bdead for trees
   ...
   subroutine fuse_2_patches(csite,donp,recp,mzg,mzs,prss,lsl,ntext_soil,green_leaf_factor &
      ...
      csite%avg_monthly_temp     (recp) = newareai                                         &
                                        * ( csite%avg_monthly_temp     (donp)              &
                                          * csite%area                 (donp)              &
                                          + csite%avg_monthly_temp     (recp)              &
                                          * csite%area                 (recp) )
      ...
      csite%avg_monthly_topwaterdiff (recp) = newareai                                     &
                                        * ( csite%avg_monthly_topwaterdiff (donp)          &
                                          * csite%area                 (donp)              &
                                          + csite%avg_monthly_topwaterdiff (recp)          &
                                          * csite%area                 (recp) )
      !------------------------------------------------------------------------------------!
      ...
      !------------------------------------------------------------------------------------!
      call update_patch_derived_props(csite,lsl, prss,recp,0)
      !------------------------------------------------------------------------------------!
