...
!------------------------------------------------------------------------------------------!
subroutine update_patch_derived_props(csite,lsl,prss,ipa,islake)
...
#if defined(COUPLED)
#else
   use lake_coms           , only : can_depth_min              ! ! intent(in)
#endif
   implicit none
   ...
   real            , intent(in)             :: prss
   integer         , intent(in), optional   :: islake
   !----- Local variables -----------------------------------------------------------------!
   ...
   if ( islakesite ) then
      !---- It's a lakesite, set the following variables to 0. ----------------------------!
      csite%veg_height(ipa)    = 0.0
      csite%veg_rough(ipa)     = 0.0
      csite%veg_displace(ipa)  = 0.0
      csite%can_depth(ipa)     = can_depth_min
