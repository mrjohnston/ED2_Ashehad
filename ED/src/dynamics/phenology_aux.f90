...
module phenology_aux
   ...
   subroutine prescribed_leaf_state(lat,imonth,iyear,doy,green_leaf_factor                 &
      ...
      real(kind=8)                        :: elonDen
      integer                             :: pft
      real(kind=8)                        :: tempvalue
      real(kind=8)                        :: greenup, maturity, senescence, dormancy
      real(kind=8)                        :: greenup2, maturity2, senescence2, dormancy2
      real(kind=8)                        :: flusha, flushb, colora, colorb
      real(kind=8)                        :: flusha2, flushb2, colora2, colorb2
      real(kind=8)                        :: decay_factor=1.001
      logical                             :: have1stQuarter
      logical                             :: have2ndQuarter
      logical                             :: have3rdQuarter
      logical                             :: have4thQuarter
      !------------------------------------------------------------------------------------!
      ...
      case (-2,1) 
          ...
          if( (lat >= 0.0 .and. imonth <= 7) .or.                                          &  
             ...
             elongf = 1.0                                                                  &
                    / (1.0 + (phen_pars%color_a(my_year)                                   &
                    * real(doy))**phen_pars%color_b(my_year))
             delay = 1.0                                                                   &
                   / (1.0 + (phen_pars%color_a(my_year) * real(doy) * decay_factor)        &
                   **phen_pars%color_b(my_year))
          end if

      case(4)
         !---------------------------------------------------------------------------------!
         !     This scheme is similar to the above scheme but allows occurence of two      !
         ! growth cycles within a year.                                                    !
         !---------------------------------------------------------------------------------!
         ...
         greenup     = phen_pars%flush_a(my_year)
         maturity    = phen_pars%flush_b(my_year)
         senescence  = phen_pars%color_a(my_year)
         dormancy    = phen_pars%color_b(my_year)
         greenup2    = phen_pars%flush_a2(my_year)
         maturity2   = phen_pars%flush_b2(my_year)
         senescence2 = phen_pars%color_a2(my_year)
         dormancy2   = phen_pars%color_b2(my_year)

         flusha      = 0.5 * ( greenup + maturity )
         flushb      = sqrt(3.0) * ( maturity - greenup ) / (6.0 * pi18)
         colora      = 0.5 * ( senescence + dormancy ) 
         colorb      = -1.0* sqrt(3.0) * ( dormancy - senescence ) / (6.0 * pi18) 
         flusha2     = 0.5 * ( greenup2 + maturity2 )
         flushb2     = sqrt(3.0) * ( maturity2 - greenup2 ) / (6.0 * pi18)
         colora2     = 0.5 * ( senescence2 + dormancy2 )
         colorb2     = -1.0 * sqrt(3.0) * ( dormancy2 - senescence2 ) / (6.0 * pi18)

         have1stQuarter = (abs(greenup)     + abs(maturity) ) > 0.5
         have2ndQuarter = (abs(senescence)  + abs(dormancy) ) > 0.5
         have3rdQuarter = (abs(greenup2)    + abs(maturity2)) > 0.5
         have4thQuarter = (abs(senescence2) + abs(dormancy2)) > 0.5

         if ( have1stQuarter .and. (.not. have2ndQuarter) .and. (.not. have3rdQuarter .or. have3rdQuarter)        &
             .and. (.not. have4thQuarter) ) then
             !--- Only have the first quarter of two growth cycles--------------------------!
             tempvalue = exp( -1.0d0 * ( real(doy) - flusha ) / flushb) 
             elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
             delay = elonDen
         else if ( (.not. have1stQuarter) .and. have2ndQuarter .and. (.not. have3rdQuarter)   &
             .and. (.not. have4thQuarter) ) then
             !--- Only have the second quarter of two growth cycles-------------------------!
             tempvalue = exp( -1.0d0 * ( real(doy) - colora ) / colorb) 
             elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
             delay = 1.0d0 / ( 1.0d0 + tempvalue * decay_factor ** abs(colorb) )
         else if ( have1stQuarter .and. have2ndQuarter .and. (.not. have3rdQuarter) .and.     &
             ((.not. have4thQuarter) .or. have4thQuarter) ) then
             !(.not. have4thQuarter) ) then
             !--- Only have the first growth cycle ----------------------------------------!
             if ( doy <= maturity) then
                tempvalue = exp( -1.0d0 * ( real(doy) - flusha ) / flushb) 
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = elonDen
             else
         ...
         else if ( have1stQuarter .and. have2ndQuarter .and. have3rdQuarter .and.             &
             (.not. have4thQuarter) ) then
             !--- Only have the first 3 quarters of the two growth cycles ------------------!
             if ( doy <= maturity) then
                tempvalue = exp( -1.0d0 * ( real(doy) - flusha ) / flushb) 
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = elonDen
             else if ( doy <= dormancy ) then
                tempvalue = exp( -1.0d0 * ( real(doy) - colora ) / colorb) 
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = 1.0d0 / ( 1.0d0 + tempvalue * decay_factor ** abs(colorb) )
             else 
                tempvalue = exp( -1.0d0 * ( real(doy) - flusha2 ) / flushb2) 
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = elonDen
             end if
         else if ( have1stQuarter .and. have2ndQuarter .and. have3rdQuarter                   &
                 .and. have4thQuarter ) then
             !--- Have the two growth cycles ----------------------------------------------!
             if ( doy <= maturity) then
                tempvalue = exp( -1.0d0 * ( real(doy) - flusha ) / flushb) 
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = elonDen
                ...
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = 1.0d0 / ( 1.0d0 + tempvalue * decay_factor ** abs(colorb) )
             else if ( doy <= maturity2 ) then
                tempvalue = exp( -1.0d0 * ( real(doy) - flusha2 ) / flushb2) 
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = elonDen
             else
                tempvalue = exp( -1.0d0 * ( real(doy) - colora2 ) / colorb2) 
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = 1.0d0 / ( 1.0d0 + tempvalue * decay_factor ** abs(colorb2) )
             end if
         else if ( (.not. have1stQuarter) .and. have2ndQuarter .and. have3rdQuarter           &
             .and. (.not. have4thQuarter) ) then
             !--- Only have the second and third quarters of two growth cycles--------------!
             if ( doy <= dormancy ) then
                tempvalue = exp( -1.0d0 * ( real(doy) - colora ) / colorb) 
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = 1.0d0 / ( 1.0d0 + tempvalue * decay_factor ** abs(colorb) )
             else
             ...
             .and. have4thQuarter ) then
             !--- Only have the second, third and fourth quarters of two growth cycles------!
             if ( doy <= dormancy ) then
                tempvalue = exp( -1.0d0 * ( real(doy) - colora ) / colorb) 
                elonDen = 1.0d0 / ( 1.0d0 + tempvalue)
                delay = 1.0d0 / ( 1.0d0 + tempvalue * decay_factor ** abs(colorb) )
             ...
             !--- Error. It shouldn't happen here. ----------------------------------------!
             write(*,*) 'greenup=',greenup,'maturity=',maturity,'senescence=',senescence,'dormancy=',dormancy
             write(*,*) 'greenup2=',greenup2,'maturity2=',maturity2,'senescence2=',senescence2,'dormancy2=',dormancy2
             call fatal_error('Problematical prescribed phenology data!',                  &
             'prescribed_leaf_state', 'phenology_aux.f90')
         end if
         ...
         if(delay < 0.0001d0) then
            delay = 0.0
         end if

         if(delay > elongf) then
            delay = elongf
         end if
   ...
   subroutine first_phenology(cgrid)
      ...
      polyloop: do ipy=1,cgrid%npolygons
         ...
         siteloop: do isi=1,cpoly%nsites
            ...
            !------------------------------------------------------------------------------!
            patchloop: do ipa=1,csite%npatches
               csite%avg_monthly_temp(ipa) = csite%can_temp(ipa)
               cpatch => csite%patch(ipa)
               cohortloop: do ico=1,cpatch%ncohorts
   ...
   subroutine pheninit_balive_bstorage(mzg,ipft,kroot,height,dbh,soil_water,ntext_soil     &
      ...
      !----- Set phenology status according to the elongation factor or green leaf factor.-!
      select case (phenology(ipft))
      case (2)
         if (green_leaf_factor(ipft) >= 1.0) then
            phenology_status = 0
         elseif (green_leaf_factor(ipft) > elongf_min) then
            phenology_status = 1
         else
            phenology_status = 2
         end if
      ...
      select case (phenology(ipft))
      case (2)
         bleaf   = bleaf_max * green_leaf_factor(ipft)
      case default
      ...
      !------------------------------------------------------------------------------------!
      !bstorage = max(0.0, (bleaf_max - bleaf)*2.0)
      bstorage = max(0.0, bleaf_max)
      select case (phenology(ipft))
      case (2)
         if (green_leaf_factor(ipft) < 0.9) bstorage = max(bstorage, balive_max)
      case default
         if (elongf < 0.9) bstorage = max(bstorage, balive_max)
      end select
      !------------------------------------------------------------------------------------!
