...
! (structural) biomass.                                                                    !
!------------------------------------------------------------------------------------------!
subroutine plant_structural_allocation(ipft,hite,dbh,lat,phen_status,bstorage,bdead,       &
                                       f_bseeds,f_bdead)
      ...
      if ((phenology(ipft) /= 2   .or.  late_spring) .and. phen_status == 0)    then
         ...
         if (is_grass(ipft)) then
            ...
            case (0)
               ...
               !--- Calculate the critical fraction to ensure that bdead+increment won't---!
               !--- exceed bdead_crit.                                                  ---!
               if (bstorage > 0.0) then
                  f_crit=min(1.0, max(0.0,(bdead_crit(ipft)-bdead)/bstorage))
               else
                  f_crit=1.0
               end if

               f_bdead=min(1.0 - st_fract(ipft) - f_bseeds,f_crit)
               f_bseeds=1.0 - st_fract(ipft) - f_bdead
               !---------------------------------------------------------------------------!
...
subroutine update_derived_cohort_props(cpatch,ico,green_leaf_factor,lsl)
   ...
   if (is_grass(ipft) .and. igrass == 1) then 
       ...
       !---- height must >= hgt_min(ipft) in case bleaf==0 (deciduous). -------------------!
