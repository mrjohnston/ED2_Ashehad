   ...
   subroutine alloc_plant_c_balance_grass(csite,ipa,ico,salloc,salloci,carbon_balance      &
      ...
      if (time_to_flush) then 
         ...
         case (0,1)
            ...
            ! to full allometry.                                                           !
            !------------------------------------------------------------------------------!
            bleaf_max      = size2bl(cpatch%dbh(ico),hgt_max(ipft),ipft)
            bleaf_aim      = bleaf_max * green_leaf_factor * cpatch%elongf(ico)
            broot_aim      = bleaf_aim * q(ipft)
            bsapwooda_aim  = bleaf_aim * qsw(ipft) * cpatch%hite(ico) * agf_bs(ipft)
            bsapwoodb_aim  = bleaf_aim * qsw(ipft) * cpatch%hite(ico) * (1. - agf_bs(ipft))
            ...
            delta_bsapwooda = max (0.0, bsapwooda_aim - cpatch%bsapwooda(ico))
            delta_bsapwoodb = max (0.0, bsapwoodb_aim - cpatch%bsapwoodb(ico))
            delta_balive    = delta_bleaf + delta_broot + delta_bsapwooda + delta_bsapwoodb
            !------------------------------------------------------------------------------!
            ...
            ! we'll put the extra into bstorage.                                           !
            !------------------------------------------------------------------------------!
            if (delta_balive > 0.0) then
               f_bleaf     = delta_bleaf     / delta_balive
               f_broot     = delta_broot     / delta_balive
               f_bsapwooda = delta_bsapwooda / delta_balive
               f_bsapwoodb = delta_bsapwoodb / delta_balive
               
               tr_bleaf     = min(delta_bleaf    , f_bleaf      * available_carbon)
               tr_broot     = min(delta_broot    , f_broot      * available_carbon)
               tr_bsapwooda = min(delta_bsapwooda, f_bsapwooda  * available_carbon)
               tr_bsapwoodb = min(delta_bsapwoodb, f_bsapwoodb  * available_carbon)
            else
               tr_bleaf    = 0.
               tr_broot    = 0.
               tr_bsapwooda = 0.
            ...
            cpatch%balive   (ico) = cpatch%bleaf(ico) + cpatch%broot(ico)                  &
                                  + cpatch%bsapwooda(ico) + cpatch%bsapwoodb(ico)
            !------------------------------------------------------------------------------!
            delta_balive    = tr_bleaf + tr_broot + tr_bsapwooda + tr_bsapwoodb
            increment       = carbon_balance - delta_balive
            ...
            !----- limited by maximum height and above minimum height. --------------------!
            cpatch%hite(ico) = max(hgt_min(ipft),                                          &
                                   min(hgt_max(ipft), bl2h(cpatch%bleaf(ico), ipft)) )
            ...
            !------------------------------------------------------------------------------!
            !   We have to recalculate balive_aim because hite has been changed            !
            !with bleaf. In other words, we have to check whether balive is equal to balive!
            !potential given the current hite.                                             !
            !------------------------------------------------------------------------------!
            bleaf_aim      = size2bl(cpatch%dbh(ico),cpatch%hite(ico),ipft)
            broot_aim      = bleaf_aim * q(ipft)
            bsapwooda_aim  = bleaf_aim * qsw(ipft) * cpatch%hite(ico) * agf_bs(ipft)
            bsapwoodb_aim  = bleaf_aim * qsw(ipft) * cpatch%hite(ico) * (1. - agf_bs(ipft))
            balive_aim     = bleaf_aim + broot_aim + bsapwooda_aim + bsapwoodb_aim
            ...
            on_allometry = 2.0 * abs(balive_aim - cpatch%balive(ico))                      &
                         / (balive_aim + cpatch%balive(ico))          < 1.e-6  .or.        &
                           cpatch%balive(ico) >= balive_aim
