!==========================================================================================!
!==========================================================================================!
subroutine copy_lake_init(sourcesite,initp)
   ...
   initp%can_co2      = dble(sourcesite%can_co2    (1))
   initp%can_theta    = dble(sourcesite%can_theta  (1))
   initp%can_theiv    = dble(sourcesite%can_theiv  (1))
   initp%can_shv      = dble(sourcesite%can_shv    (1))
   initp%lake_temp    = dble(sourcesite%ground_temp(1))
   ...
   initp%can_exner    = cpdry8 * (initp%can_prss * p00i8) ** rocp8

   initp%can_temp     = cpdryi8 * initp%can_theta * initp%can_exner

   initp%can_rhos     = idealdenssh8(initp%can_prss,initp%can_temp,initp%can_shv)
   initp%can_rhv      = rehuil8(initp%can_prss,initp%can_temp,initp%can_rvap,.false.)
   ...
   initp%lake_fliq    = sourcesite%soil_fracliq(nzg,1)
   ...
   !---------------------------------------------------------------------------------------!
   ustar8             = max(ustmin8,dble(sourcesite%ustar(1)))
   ...
   !---------------------------------------------------------------------------------------!
   call can_whccap8(initp%can_rhos,initp%can_temp                                          &
   ...
   if(fast_diagnostics) then
      initp%avg_rshort_gnd        = dble(sourcesite%avg_rshort_gnd  (1))
      initp%avg_rlong_gnd         = dble(sourcesite%avg_rlong_gnd   (1))
      initp%avg_carbon_ac         = dble(sourcesite%avg_carbon_ac   (1))
      initp%avg_carbon_gc         = 0.d0 
      initp%avg_sensible_gc       = dble(sourcesite%avg_sensible_gc (1))
      initp%avg_sensible_ac       = dble(sourcesite%avg_sensible_ac (1))
      initp%avg_vapor_gc          = dble(sourcesite%avg_vapor_gc    (1))
      initp%avg_vapor_ac          = dble(sourcesite%avg_vapor_ac    (1))
   end if


   if(checkbudget) then
      initp%co2budget_storage     = dble(sourcesite%co2budget_initialstorage(1))
      initp%ebudget_storage       = dble(sourcesite%ebudget_initialstorage(1)  )
      initp%wbudget_storage       = dble(sourcesite%wbudget_initialstorage(1)  )
      initp%co2budget_loss2atm    = 0.d0
      initp%ebudget_loss2atm      = 0.d0
      initp%ebudget_loss2drainage = 0.d0
      initp%ebudget_loss2runoff   = 0.d0
      initp%wbudget_loss2atm      = 0.d0
      initp%wbudget_loss2drainage = 0.d0
...
subroutine lake_diagnostics(initp)
   ...
   if (.not. ok_flpoint) then
      ...
      write(unit=*,fmt='(a)'          ) ' '
      write(unit=*,fmt='(a)'          ) '-------------------------------------------------'
      call fatal_error('Non-resolvable values','lake_diagnostics','ed_lake_misc.f90')
   end if
   ...
   ok_theta  = initp%can_lntheta >= log(rk4min_can_theta) .and.                             &
               initp%can_lntheta <= log(rk4max_can_theta)
...
!------------------------------------------------------------------------------------------!
subroutine lake_derivs(initp,dinitp,csite)
   use lake_coms             , only : lakesitetype         & ! structure  
                                    , lakemet              & ! intent(in) 
                                    , hcapcani             & ! intent(in) 
                                    , wcapcani             & ! intent(in) 
   ...
   eflxac    = rho_ustar * initp%estar * cpdry8 * initp%can_temp   ! Enthalpy flux
   !---------------------------------------------------------------------------------------!
   ...
   hflxgc    = cpdry8 * initp%can_rhos * initp%gglake * (initp%lake_temp - initp%can_temp)
   ...
   if (fast_diagnostics .or. checkbudget .or. print_detailed) then
      ...
      dinitp%avg_rshort_gnd    = dble(csite%rshort_g(1))
      ...
      dinitp%avg_rlong_gnd = dble(csite%rlong_g(1)) + dble(csite%rlong_s(1))
      !------------------------------------------------------------------------------------!
   end if
   !---------------------------------------------------------------------------------------!
   ...
   if (checkbudget) then
      ...
      dinitp%co2budget_storage  = cflxgc + cflxac
      dinitp%ebudget_storage    = eflxac + dinitp%avg_rshort_gnd + dinitp%avg_rlong_gnd
      dinitp%wbudget_storage    = wflxac
   end if
   !---------------------------------------------------------------------------------------!
   return
...
subroutine lake_derived_thbounds(can_rhos,can_theta,can_temp,can_shv,can_rvap,can_prss     &
   ...
   can_theiv_try    = thetaeivs8(can_theta_try,can_temp,                                   &
                                rk4max_can_shv / (1.d0 - rk4max_can_shv),0.d0,0.d0)
   !rk4max_can_theiv = max(rk4max_can_theiv,can_theiv_try)
   !---------------------------------------------------------------------------------------!
...
subroutine lake_sanity_check(y,reject_step,dydx,h,print_problems)
   ...
   !---------------------------------------------------------------------------------------!
   if (y%can_theiv > rk4max_can_theta .or. y%can_theiv < rk4min_can_theta ) then
      reject_step = .true.
   ...
   if (reject_step .and. print_problems) then
      ...
      write(unit=*,fmt='(6(es12.5,1x))')  rk4min_can_theta,rk4max_can_theta             &
...
subroutine print_lakesite(y,initp,htry)
   ...
   call fatal_error('IFLAG1 problem. The model didn''t converge!','print_lakesite'           &
                 ,'ed_lake_misc.f90')
   return
