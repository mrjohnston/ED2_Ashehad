...
subroutine calcWatertable(cpoly,isi,ipa)
  ...
  use consts_coms, only: cliqvlme,tsupercool_liq,cicevlme
  implicit none
  type(sitetype)   , pointer :: csite
  ...
  layerloop: do k = cpoly%lsl(isi),nzg
     ...
     !for a lake site, soil_water=1.0. Therefore, we have to make sure soil_water/slmsts <=1.0
     fracw = min(1.0, csite%soil_water(k,ipa) / soil(nsoil)%slmsts)
     ...
     if ( cpoly%islakesite(isi) ==1 ) then
        csite%soil_sat_heat(ipa)  = csite%soil_sat_heat(ipa) + dslz(k)                                &
                                    * ( (1.0-csite%soil_fracliq(k,ipa)) * cicevlme                    &
                                    * csite%soil_tempk(k,ipa) + csite%soil_fracliq(k,ipa) * cliqvlme  &
                                    * ( csite%soil_tempk(k,ipa) - tsupercool_liq ) ) 
     else
