...
subroutine update_phenology(doy, cpoly, isi, lat)
   ...
   use pft_coms       , only : phenology                & ! intent(in)
                             ...
                             , is_grass                 & ! intent(in)
                             , hgt_max                  & ! intent(in)
                             , flushing_temp_min        & ! intent(in)
                             , flushing_temp_max        & ! intent(in)
                             , dl_min                   & ! intent(in)
                             , dl_max                   ! ! intent(in)
      ...
      cohortloop: do ico = 1,cpatch%ncohorts
         ...
         case (2)
            ...
            elseif (cpatch%phenology_status(ico) == 2 .and. leaf_out_cold) then
               ...
               cpatch%elongf          (ico) = 1.0 ! It should become green_leaf_factor...
               !---------------------------------------------------------------------------!
            elseif (cpatch%phenology_status(ico) == 0 .and. cpatch%bleaf(ico) < bl_max) then
               cpatch%phenology_status(ico) = 1
            end if
         ...
         case (3,4,5) 
            !------------------------------------------------------------------------------!
            !    Drought deciduous/Drought+Cold deciduous.                                 !
            ! Here we must check two possibilities:                                        !
            !                                                                              !
            ! 1. The soil has been dry/cold recently, and the plants have leaves, so we    !
            !    flag them with phenology_status=0 (leaves not growing) and the plants     !
            !    will start losing their leaves;                                           !
            ! 2. The plant has no leaves, but the soil has started to come back to more    !
            !    moist/warm conditions. Given this situation, leaves can start growing     !
            !    again.                                                                    !
            !------------------------------------------------------------------------------!
            !----- This is the first guess for the new elongation factor. -----------------!
            if (phenology(ipft) == 5) then
                elongf_temp   = min(1.0, max(0.0, (csite%avg_monthly_temp(ipa) -           &
                                flushing_temp_min(ipft)) / (flushing_temp_max(ipft) -      &
                                flushing_temp_min(ipft) ) ) )
                elongf_photop = min(1.0, max(0.0, (daylight - dl_min(ipft)) /              &
                                ( dl_max(ipft) - dl_min(ipft) ) ) ) 
                elongf_try    = max(0.0, min (1.0, cpatch%paw_avg(ico)))
                elongf_try    = elongf_try * elongf_temp * elongf_photop
            else
                elongf_try  = max(0.0, min (1.0, cpatch%paw_avg(ico)))
            end if
            ...
            if(is_grass(ipft).and. igrass==1) then 
                !-- For new grass scheme, hite is updated with bleaf so that elongf       -!
                !-- shouldn't be multiplied to the max bleaf of the current size because  -!
                !-- it is on "max". Instead, bl_max should be equal to elongf_try *       -!
                !-- size2bl(maxdbh, hgt_max)                                              -!
                bl_max = elongf_try * size2bl(cpatch%dbh(ico),hgt_max(ipft),ipft)
            else
            ...
            elseif (cpatch%phenology_status(ico) /= 0) then
               ...
               !---------------------------------------------------------------------------!
            elseif (cpatch%phenology_status(ico) == 0 .and. delta_bleaf < 0.0) then
               !---------------------------------------------------------------------------!
               !  Leaves are not on maximum. Should set the status to growing.             !
               !---------------------------------------------------------------------------!
               cpatch%phenology_status(ico) = 1 
            end if
            !------------------------------------------------------------------------------!
...
subroutine assign_prescribed_phen(green_leaf_factor,leaf_aging_factor,dbh,height,pft       &
   ...
   use ed_misc_coms  , only : igrass
   use pft_coms      , only : is_grass, hgt_max
   implicit none
   !------ Arguments. ---------------------------------------------------------------------!
   logical, intent(out) :: drop_cold
   logical, intent(out) :: leaf_out_cold
   ...
   if(is_grass(pft).and. igrass==1) then                                                 
       !-- For new grass scheme, hite is updated with bleaf so that greenleaf factor       !
       !-- shouldn't be multiplied to the max bleaf of the current size because            !
       !-- it is on "max". Instead, bl_max should be equal to greenleaf factor *           !
       !-- size2bl(dbh, hgt_max)                                                           !
       bl_max = green_leaf_factor * size2bl(dbh,hgt_max(pft),pft)
   else
