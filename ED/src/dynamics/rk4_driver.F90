...
module rk4_driver
   ...
   subroutine initp2modelp(hdid,initp,csite,ipa,wbudget_loss2atm,ebudget_netrad            &
      ...
      use pft_coms             , only : dry_period           ! ! intent(in)
      implicit none
      !----- Arguments --------------------------------------------------------------------!
      type(rk4patchtype), target      :: initp
      type(sitetype)    , target      :: csite
      ...
      csite%avg_monthly_temp(ipa) = csite%avg_monthly_temp(ipa)                            &
                                    *(1.0-sngl(hdid)/thirtydays_sec)                       &
                                    + csite%can_temp(ipa)*sngl(hdid)/thirtydays_sec
      !------------------------------------------------------------------------------------!
      ...
      !------------------------------------------------------------------------------------!
      !     This variable is the monthly mean top 1m water content that will be used to    !
      ! control seeds germination.                                                         !
      !------------------------------------------------------------------------------------!
      gnd_water = 0.d0
      depth = 0.d0
      !----- Find the bottommost layer to consider. ---------------------------------------!
      ...
      do k=ka,nzg
         ...
         gnd_water = gnd_water + ( initp%soil_water(k) - soil8(nsoil)%soilld ) * dslz8(k)
         depth = depth + dslz8(k)
      end do
      !----- Add to the monthly mean. -----------------------------------------------------!
      csite%avg_monthly_topwaterdiff(ipa) = csite%avg_monthly_topwaterdiff(ipa)            &
                                      + sngloff(gnd_water/depth,tiny_offset)
      !------------------------------------------------------------------------------------!



      !------------------------------------------------------------------------------------!
      ! paw_avg - dry_period days average of relative plant available water.  The relative !
      !           value depends on whether the user wants to define phenology based on soil!
      ...
      if (spot_phen) then
         ...
         do ico = 1,cpatch%ncohorts
            ...
            cpatch%paw_avg(ico) = cpatch%paw_avg(ico)*(1.0-sngl(hdid)/dry_period(ipft)     &
                                / day_sec) + sngl(available_water)*sngl(hdid)              &
                                /dry_period(ipft)/day_sec
         end do
      else
         cpatch => csite%patch(ipa)
         do ico = 1,cpatch%ncohorts
