               ...
               new_lu_loop: do new_lu = 1, n_dist_types
                  ...
                  if (area > min_patch_area) then
                     ...
                     call update_patch_derived_props( csite,cpoly%lsl(isi)                 &
                                                    , cpoly%met(isi)%prss                  &
                                                    , new_lu+onsp,cpoly%islakesite(isi))
      ...
      case (1)
         ...
         polyloop2: do ipy = 1,cgrid%npolygons
            ...
            siteloop2: do isi = 1,cpoly%nsites
               ...
               csite => cpoly%site(isi)
               !---- Ignore the lake site. ------------------------------------------------!
               if ( cpoly%islakesite(isi)==1 ) cycle siteloop2
   ...
   subroutine site_disturbance_rates(month, year, cgrid)
      ...
      polyloop: do ipy = 1,cgrid%npolygons
         ...
         siteloop: do isi = 1,cpoly%nsites
            ...
            !------------------------------------------------------------------------------!
            !   For a lake site, all disturbances (natural and anthropogenic) are set to   !
            ! zeros for the moment.                                                        !
            !------------------------------------------------------------------------------!
            if ( cpoly%islakesite(isi) == 1 ) then
               cpoly%fire_disturbance_rate(isi) = 0.0
               cpoly%nat_dist_type(isi)         = 0
               cpoly%nat_disturbance_rate(isi)  = 0.0
