...
subroutine get_work(ifm,nxp,nyp)
   ...
   if (isoilflg(ifm) == 3) then
      call read_mask(trim(veg_database(ifm)),maxsite,npoly,lat_list,lon_list,ipcent_land)
   else
   ...
   elseif (isoilflg(ifm) == 3) then
      call read_soilinfo(trim(soil_database(ifm)),maxsite,npoly,lat_list,lon_list,ntext_soil_list)
      ipcent_soil            (:,:) = 0.
      ipcent_soil            (1,:) = 1.
   else
      !------------------------------------------------------------------------------------!
