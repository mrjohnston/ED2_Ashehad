...
subroutine ed_model()
   ...
   use soil_coms     , only : n_soilwater_points               & ! intent(in)
   ...
   timestep: do while (time < timmax)
...
#if defined(COUPLED)
#else
      !----- Solve the fluxes of lake/water sites -----------------------------------------!
      ...
      new_hour        = mod(current_time%time, 3600.0) < dtlsm
      ...
      !------------------------------------------------------------------------------------!
      !   Read prescribed soil water if available.                                         !
      !------------------------------------------------------------------------------------!
      if (n_soilwater_points == -1  .and. (new_hour .or. istp==1) ) then
         !---------------------------------------------------------------------------------!
         !   Prescribed soil water are  hourly continuous                                  !
         !---------------------------------------------------------------------------------!
         time2readsm = ( (current_time%year>soilwater_years(1) .and.                       &
                            current_time%year<soilwater_years(2)) )                 .or.   &
                       ( (current_time%year==soilwater_years(1)) .and.                     &
                         ( (current_time%month>soilwater_months(1))  .or.                  &
                           ( current_time%month==soilwater_months(1) .and.                 &
                             current_time%date>=soilwater_dates(1) ) ) )            .or.   &
                       ( (current_time%year==soilwater_years(2)) .and.                     &
                         ( (current_time%month<soilwater_months(2))  .or.                  &
                           ( current_time%month==soilwater_months(2) .and.                 &
                             current_time%date<=soilwater_dates(2) ) ) )

         if (time2readsm) then
             readdata=(istp==1 .or. current_time%hour==0)
             do ifm=1,ngrids
                call read_continuous_soilwater(edgrid_g(ifm),ifm,readdata,current_time%hour+1)
                !call read_soilwater(edgrid_g(ifm),ifm)
             end do
         end if
      end if
      ...
      if (new_day) then
         ...
         !   Read prescribed soil water if available.                                      !
         !---------------------------------------------------------------------------------!
         if (n_soilwater_points >0) then
            !------------------------------------------------------------------------------!
            !   Prescribed soil water are discrete                                         !
            !------------------------------------------------------------------------------!
            time2readsm=.false.
            n_loop: do n=1,n_soilwater_points
                if( current_time%year == soilwater_years(n) .and.                          &
                    current_time%month == soilwater_months(n) .and.                        &
                    current_time%date == soilwater_dates(n) ) then
                   time2readsm=.true.
                   exit n_loop
                end if
            end do n_loop
      ...
      if (analysis_time .and. new_month .and. new_day .and. current_time%month == 7) then
         do ifm = 1,ngrids
            call update_ed_yearly_vars(edgrid_g(ifm))
         end do
      end if
