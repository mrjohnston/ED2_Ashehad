...
!==========================================================================================!
!==========================================================================================!
!     This subroutine is the procedure to read prescribed soil water content.              !
!------------------------------------------------------------------------------------------!
subroutine read_soilwater(cgrid,ifm)
   use grid_coms              , only   : nzg                  ! ! intent(in)
   use ed_state_vars          , only   : edtype               & ! structure
                                       , polygontype          & ! structure
                                       , sitetype             ! ! structuree
   use ed_misc_coms           , only   : current_time         ! ! intent(in)
   use soil_coms              , only   : soilwater_db         & ! intent(in)
                                       , soil                 ! ! intent(in)
   use ed_misc_coms           , only   : current_time         ! ! intent(in)
   ...
   integer                                          :: nzg_soil
   real                                             :: site_area_i
   character(len=str_len)                           :: soilwater_file, reason
   logical                                          :: l1, l2
   integer                                          :: ndims
   integer   , dimension(2)                         :: idims2D, idims2D0
   integer   , dimension(3)                         :: idims3D
   real      , dimension(:,:)  , allocatable        :: lat2d
   real      , dimension(:,:)  , allocatable        :: lon2d
   real      , dimension(:,:,:), allocatable        :: tmp_data
   real                                             :: lat0, lon0
   integer                                          :: nlon, ilon
   integer                                          :: nlat, ilat, nsoil, isoil
   real                                             :: min_dist, latmin, latmax
   real                                             :: this_dist, lonmin, lonmax
   real                                             :: weight
   !----- External function ------------------------------------------------------------!
   real                        , external           :: dist_gc
   !------------------------------------------------------------------------------------!
   ...
   !  Set the soilwater file name
   write(soilwater_file,'(a,i4.4,2(a,i2.2),a)') trim(soilwater_db), current_time%year,     &
           '-', current_time%month, '-', current_time%date, '.h5'
           
   inquire(file=trim(soilwater_file), exist=l1, opened=l2)
   
   if (.not. l1) then
       write(reason, '(3a)')  'File ', trim(soilwater_file), 'doesn''t exist!'
       call fatal_error(reason,'read_soilwater','ed_met_driver.f90')
   end if
  
   !if (mynum ==1) write(*,'(3a)') 'Reading ', trim(soilwater_file), '...' 
   write(*,'(3a)') 'Reading ', trim(soilwater_file), '...' 
   !----- Read lat and lon first ----------------------------------------------------------!
   call shdf5_open_f(trim(soilwater_file),'R')

   !  Get the dimensioning information on latitude
   call shdf5_info_f('lat',ndims,idims2D)
   if(ndims /= 2) then
      write(reason,fmt='(2a,1x,i5,2a)') 'Number of dimensions of latitude is wrong...',    &
                                        'NDIMS=',ndims, ' in ', trim(soilwater_file)
      call fatal_error (reason, 'read_soilwater', 'ed_met_driver.f90')
   endif
   ...
   nlat = idims2D(2)
   !  Allocate the latitude array
   allocate(lat2d(idims2D(1),idims2D(2)))
   allocate(lon2d(idims2D(1),idims2D(2)))

   !  Read in the latitude array
   call shdf5_irec_f(ndims, idims2D, 'lat', rvara = lat2d )

   !  Get the dimensioning information on longitude
   call shdf5_info_f('lon',ndims,idims2D)

   if(idims2D(1) /= nlon .or. idims2D(2) /=nlat) then
      write(reason,fmt='(2a)')                                                          &
               'Dimensions of longitude doesnot match with those of latitude in !',     &
               trim(soilwater_file)
      call fatal_error (reason,'get_griddata_location','phenology_startup.f90')
   endif

   !  Read in the latitude array
   call shdf5_irec_f(ndims, idims2D, 'lon', rvara = lon2d )
   do ilat = 1,nlat
      do ilon = 1,nlon
   ...
   call shdf5_info_f('soilwater',ndims,idims3D)
   nzg_soil=idims3D(3)
   if(ndims /= 3) then
      write(reason,fmt='(2a,1x,i5,2a)') 'Number of dimensions of soilwater is wrong...',  &
                                        'NDIMS=',ndims, ' in ', trim(soilwater_file)
      call fatal_error (reason, 'read_soilwater', 'ed_met_driver.f90')
   endif
   
   if(nzg_soil /= nzg) then
      write(reason,fmt='(2a,1x,i5,2a)') 'Number of soil layers is wrong...',              &
                                        'NZG=',nzg_soil, ' in ', trim(soilwater_file)
      call fatal_error (reason, 'read_soilwater', 'ed_met_driver.f90')
   endif
   
   allocate(tmp_data(idims3D(1),idims3D(2),idims3D(3)))
   call shdf5_irec_f(ndims, idims3D, 'soilwater', rvara = tmp_data )
   ...
   polygonloop: do ipy = 1,cgrid%npolygons
      ...
      lon0=cgrid%lon(ipy)
      min_dist = huge (1.)
      if ( lat0 < latmin .or. lat0 > latmax .or.                                          &
           lon0 < lonmin .or. lon0 > lonmax ) then
           write(*,fmt='(a,2(f10.5,1x),2a)')                                              &
              'Warning: one polygon (', lon0, lat0,                                       &
              ') is out of the range of your provided soil water data from '              &
             , trim(soilwater_file)
            exit polygonloop                        
           !call fatal_error (reason,'read_soilwater','ed_met_driver.f90')
      end if
      ...
      lonloop: do ilon = 1,nlon
         latloop: do ilat = 1,nlat
            this_dist = dist_gc(lon0, lon2d(ilon,ilat) ,lat0, lat2d(ilon,ilat) )
      ...
      isoilloop: do isoil = cgrid%lsl(ipy),nzg
         if (tmp_data(idims2D0(1),idims2D0(2),isoil)>0) then
            siteloop: do isi = 1,cpoly%nsites
               csite => cpoly%site(isi)
               site_area_i = 1./sum(csite%area)
               weight=tmp_data(idims2D0(1),idims2D0(2),isoil) / (site_area_i*              &
                        sum(csite%area*csite%soil_water(isoil,:)))
               nsoil=cpoly%ntext_soil(isoil,isi)
              
               ipaloop: do ipa=1,csite%npatches
                  csite%soil_water(isoil,ipa)=min(soil(nsoil)%slmsts,                      &
                                                  max(soil(nsoil)%soilcp,                  &
                                                  csite%soil_water(isoil,ipa)*weight) )
                  csite%soil_energy(isoil,ipa)  = cmtl2uext( soil(nsoil)%slcpd             &
                                                  , csite%soil_water(isoil,ipa)*wdns       &
                                                  , csite%soil_tempk(isoil,ipa)            &
                                                  , csite%soil_fracliq(isoil,ipa) )
               end do ipaloop
            end do siteloop
         end if
      end do isoilloop
...
!     This subroutine is the procedure to read prescribed soil water content.              !
!------------------------------------------------------------------------------------------!
subroutine read_continuous_soilwater(cgrid,ifm,readdata,hr)
   use grid_coms              , only   : nzg                  ! ! intent(in)
   use ed_state_vars          , only   : edtype               & ! structure
                                       , polygontype          & ! structure
                                       , sitetype             ! ! structuree
   ...
   !----- Arguments -----------------------------------------------------------------------!
   type(edtype)                , target               :: cgrid
   integer                     , intent (in)          :: ifm, hr
   logical                     , intent (in)          :: readdata
   !----- Local variables -----------------------------------------------------------------!
   type(polygontype)           , pointer              :: cpoly
   type(sitetype)              , pointer              :: csite
   integer                                            :: ipy
   ...
   integer                                            :: nzg_soil
   real                                               :: site_area_i
   character(len=str_len)      , save                 :: soilwater_file, reason
   logical                                            :: l1, l2
   integer                                            :: ndims
   integer   , save, dimension(2)                     :: idims2D, idims2D0
   integer   , save, dimension(4)                     :: idims4D
   real      , save, dimension(:,:)  , allocatable    :: lat2d
   real      , save, dimension(:,:)  , allocatable    :: lon2d
   real      , save, dimension(:,:,:,:), allocatable  :: tmp_data
   real                                               :: lat0, lon0
   integer   , save                                   :: nlon, nlat
   integer                                            :: ilon, ilat, nsoil, isoil, nhrs
   real      , save                                   :: latmin, latmax, lonmin, lonmax
   real                                               :: min_dist,this_dist
   real                                               :: weight
   !----- External function ------------------------------------------------------------!
   real                        , external             :: dist_gc
   !------------------------------------------------------------------------------------!
   
   if (readdata) then !In this case, we need read data
       !----- Free memory so it won't leak it. -----------------------------------------!
       ...
       if (.not. l1) then
           write(reason, '(3a)')  'File ', trim(soilwater_file), 'doesn''t exist!'
           call fatal_error(reason,'read_continuous_soilwater','ed_met_driver.f90')
       end if
       ...
       call shdf5_info_f('soilwater',ndims,idims4D)
       nzg_soil=idims4D(3)
       nhrs=idims4D(4)
       if(ndims /= 4) then
          write(reason,fmt='(2a,1x,i5,2a)') 'Number of dimensions of soilwater is wrong...',  &
       ...
       if(nhrs /= 24) then
          write(reason,fmt='(2a,1x,i5,2a)') 'Number of hours is wrong...',                    &
                                            'nhrs=',nhrs, ' in ', trim(soilwater_file)
          call fatal_error (reason, 'read_continuous_soilwater', 'ed_met_driver.f90')
       endif
       
       allocate(tmp_data(idims4D(1),idims4D(2),idims4D(3),idims4D(4)))

       call shdf5_irec_f(ndims, idims4D, 'soilwater', rvara = tmp_data )
   ...
   polygonloop: do ipy = 1,cgrid%npolygons
      ...
      if ( lat0 < latmin .or. lat0 > latmax .or.                                          &
           lon0 < lonmin .or. lon0 > lonmax ) then
           write(reason,fmt='(a,2(f10.5,1x),2a,4(f10.5,1x))')                             &
              'Warning: one polygon (', lon0, lat0,                                       &
              ') is out of the range of your provided soil water data from '              &
             , trim(soilwater_file), latmin, latmax, lonmin, lonmax
            exit polygonloop                        
           call fatal_error (reason,'read_continuous_soilwater','ed_met_driver.f90')
      end if
      ...
      isoilloop: do isoil = cgrid%lsl(ipy),nzg
         if (tmp_data(idims2D0(1),idims2D0(2),isoil,hr)>0) then
            siteloop: do isi = 1,cpoly%nsites
               csite => cpoly%site(isi)
               site_area_i = 1./sum(csite%area)
               weight=tmp_data(idims2D0(1),idims2D0(2),isoil,hr) / (site_area_i*              &
                        sum(csite%area*csite%soil_water(isoil,:)))
               nsoil=cpoly%ntext_soil(isoil,isi)
...
end subroutine read_continuous_soilwater
!==========================================================================================!
!==========================================================================================!
