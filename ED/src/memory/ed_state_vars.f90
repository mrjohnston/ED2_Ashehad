...
!============================================================================!
  ...
  type sitetype
     ...
     ! Average 30day temperature [K]
     real , pointer,dimension(:) :: avg_monthly_temp  
     ...
     ! Average monthly difference between soil water content and soilld for the
     ! top 1m [m3/m3], used for seeds germination
     real , pointer,dimension(:) :: avg_monthly_topwaterdiff
...
!============================================================================!
  ...
  type polygontype
     ...
     ! Flag to mark whether the site is a lake(water) site
     integer,pointer,dimension(:) :: islakesite
...
!============================================================================!
  subroutine allocate_polygontype(cpoly,nsites)
    ...
    allocate(cpoly%islakesite(nsites))
...
!============================================================================!
  subroutine allocate_sitetype(csite,npatches)
    ...
    allocate(csite%avg_monthly_temp(npatches))  
    allocate(csite%avg_monthly_gndwater(npatches))  
    allocate(csite%avg_monthly_waterdef(npatches))  
    allocate(csite%avg_monthly_topwaterdiff(npatches))  
...
!============================================================================!
  subroutine nullify_sitetype(csite)
    ...
    nullify(csite%avg_monthly_temp)
    nullify(csite%avg_monthly_gndwater)
    nullify(csite%avg_monthly_waterdef)
    nullify(csite%avg_monthly_topwaterdiff)
...
!============================================================================!
  subroutine deallocate_polygontype(cpoly)
    ...
    if(associated(cpoly%islakesite                  )) deallocate(cpoly%islakesite                  )
...
!============================================================================!
  subroutine deallocate_sitetype(csite)
    ...
    if(associated(csite%avg_monthly_temp             )) deallocate(csite%avg_monthly_temp             )
    ...
    if(associated(csite%avg_monthly_topwaterdiff     )) deallocate(csite%avg_monthly_topwaterdiff     )
...
!============================================================================!
   ...
   subroutine copy_sitetype(isite,osite,ipaa,ipaz,opaa,opaz)
      ...
      opaloop: do opa = opaa, opaz
         ...
         osite%avg_monthly_temp(opa)            = isite%avg_monthly_temp(ipa)
         ...
         osite%avg_monthly_topwaterdiff(opa)    = isite%avg_monthly_topwaterdiff(ipa)
...
!============================================================================!
  subroutine copy_sitetype_mask(sitein,siteout,logmask,masksz,newsz)
    ...
    siteout%avg_monthly_temp(1:inc)     = pack(sitein%avg_monthly_temp,logmask)
    ...
    siteout%avg_monthly_topwaterdiff(1:inc) = pack(sitein%avg_monthly_topwaterdiff,logmask)
...
!==========================================================================================!
   ...
   subroutine filltab_polygontype(igr,ipy,init)
      ...
      if (associated(cpoly%islakesite)) then
         nvar=nvar+1
         call vtable_edio_i(npts,cpoly%islakesite,nvar,igr,init,cpoly%siglob_id, &
              var_len,var_len_global,max_ptrs,'ISLAKESITE :20:hist:anal:dail:mont:dcyc:year')
         call metadata_edio(nvar,igr,'Flag to mark whether this site is a lake/water site','[NA]','NA')
      end if
   ...
   subroutine filltab_sitetype(igr,ipy,isi,init)
      ...
      if (associated(csite%avg_monthly_temp)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%avg_monthly_temp,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'AVG_MONTHLY_TEMP :31:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if  
      ...
      if (associated(csite%avg_monthly_topwaterdiff)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%avg_monthly_topwaterdiff,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'AVG_MONTHLY_TOPWATERDIFF :31:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if  
   ...
   subroutine filltab_patchtype(igr,ipy,isi,ipa,init)
      ...
      if (associated(cpatch%leaf_energy)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'LEAF_ENERGY :41:hist:dail:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpatch%leaf_hcap)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'LEAF_HCAP :41:hist:dail:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpatch%leaf_temp)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'LEAF_TEMP :41:hist:dail:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpatch%leaf_vpdef)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'LEAF_VPDEF :41:hist:dail:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpatch%leaf_fliq)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'LEAF_FLIQ :41:hist:dail:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpatch%leaf_water)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'LEAF_WATER :41:hist:dail:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
