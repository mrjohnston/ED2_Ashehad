...
#else
   !---------------------------------------------------------------------------------------!
   !   This is a flag to tell the model whether allocate a lakesite for a polygon where    !
   ! there is a fraction of water body falling in the polygon. This flag should be ignored !
   ! for coupled simulations because the coupled model has its own procedures to deal with !
   ! lakesiite. 
   ! 0 - don't allocate a lakesite; 1 - derive water fraction from VEG_DATABASE and        !
   ! allocate a lakesite; 2 - water fraction is provided in the namelist and allocate a    !
   ! lakesite.                                                                             !     
   !---------------------------------------------------------------------------------------!
   integer :: include_lakesite
   real    :: waterfrac    !Fraction of water
#endif

   !---------------------------------------------------------------------------------------!
   !    This is the maximum allowed number of land sites for a simulation.  This is a      !
   ! strict maximum, but the actual number of land sites can be less in case there aren't  !
   ! enough different types of soil.                                                       !
   !---------------------------------------------------------------------------------------!
   integer :: maxsite
