...
subroutine set_site_defprops()
   ...
   gridloop: do ifm=1,ngrids
      ...
      polyloop: do ipy=1,cgrid%npolygons
         ...
         !---------------------------------------------------------------------------------!
         nlandsite = min(maxsite, count(work_v(ifm)%soilfrac(:,ipy) > min_site_area))
         text_area = sum(work_v(ifm)%soilfrac(1:nlandsite,ipy))
         !----- Sanity check. -------------------------------------------------------------!
         if (nlandsite == 0 .or. text_area <= 0.) then
            write (unit=*,fmt='(a)'          ) '------------------------------------------'
            write (unit=*,fmt='(a,1x,i12)'   ) ' + NSITE     = ',nlandsite
         ...
         !---------------------------------------------------------------------------------!
         !    Here we decide whether we need add a lakesite. Right now, we only define     !
         ! one lake site. In the future, we may define multiple lake sites.                !
         !---------------------------------------------------------------------------------!
#if defined(COUPLED) 
         !-------- In the coupled model, we don't need add lake site. ---------------------!
         nlakesite = 0
         landfrac  = 1.0
#else
         select case (include_lakesite)
         case (0)
            !----- Don't create a lakesite. All fractions are land ------------------------!
            landfrac  = 1.0
         case (1)
            !----- We likely need create a lakesite and must determine waterfrac from -----!
            !----- work_v. ----------------------------------------------------------------!
            landfrac  = work_v(ifm)%landfrac(ipy)
         case default
            !----- We likely need create a lakesite and must use waterfrac from namelist --!
            landfrac  = 1.0 - waterfrac
         end select

         if ( (1.0 - landfrac) > min_site_area ) then
            nlakesite = 1
         else
            nlakesite = 0
            landfrac  = 1.0
         ...
         !---------------------------------------------------------------------------------!
         !     Initialize the lake site.                                                   !
         !---------------------------------------------------------------------------------!
         isi_prev = isi
         siteloop2: do itext=1,nlakesite
            if ( (1.0 - landfrac) > min_site_area) then
               !----- Update counter. -----------------------------------------------------!
               isi = isi +1
               cpoly%islakesite(isi) = 1

               !---------------------------------------------------------------------------!
               !     Initialise area.                                                      !
               !---------------------------------------------------------------------------!
               cpoly%area(isi) = 1.0 - landfrac
               !---------------------------------------------------------------------------!
               ...
               !---------------------------------------------------------------------------!
               !     Use the soil type of the previous land site.                          !
               !---------------------------------------------------------------------------!
               do k=1,nzg
                  cpoly%ntext_soil(k,isi) = cpoly%ntext_soil(k,isi_prev)
               end do
               !---------------------------------------------------------------------------!
...
subroutine soil_depth_fill(cgrid,igr)
   ...
   integer        , dimension(3)                           :: idims2
   integer        , dimension(:,:)    , allocatable        :: idims0
   integer                                                 :: ilon
   integer                                                 :: ilat
   integer                                                 :: nlon
   integer                                                 :: nlat
   ...
   real           , dimension(:,:,:)  , allocatable        :: slz_tmp
   ...
   if (isoildepthflg == 2) then
      !----- Read lat and lon first -------------------------------------------------------!
      call shdf5_open_f(trim(soildepth_db),'R')
      ...
      if(ndims /= 2) then
         ...
         call fatal_error ('Not set up to have time varying latitude...' &
                          ,'get_griddata_location','phenology_startup.f90')
      endif
      ...
      allocate(idims0(cgrid%npolygons,2))
      
      !  Determine the indices of the grid that each polygon corresponds to.
      do ipy = 1,cgrid%npolygons
          min_dist = huge (1.)
          if ( cgrid%lat(ipy) < latmin .or. cgrid%lat(ipy) > latmax .or.                 &
               cgrid%lon(ipy) < lonmin .or. cgrid%lon(ipy) > lonmax ) then
               write(unit=*,fmt='(2a)')                                                    &
                  'At least one polygon is out of the range of your provided soil data from'&
                 , trim(soildepth_db)

               call fatal_error ('One polygon is out of the range of your provided soil data...'  &
                          ,'soil_depth_fill','ed_init.F90')
          end if
      ...
      allocate(nz(idims(1),idims(2)))
      call shdf5_irec_f(ndims, idims, 'nzg', ivara = nz )
     
      call shdf5_info_f('slz',ndims,idims2)
      allocate(slz_tmp(idims2(1),idims2(2),idims2(3)))
      call shdf5_irec_f(ndims, idims2, 'slz', rvara = slz_tmp )
      
      
      do ipy = 1,cgrid%npolygons
         cgrid%lsl(ipy) = nzg-1
         do k = nzg-2,1,-1
             if (slz_tmp(nz(idims0(ipy,1),idims0(ipy,2)),idims0(ipy,1),idims0(ipy,2)) < slz(k+1)) then
                cgrid%lsl(ipy) = k
             end if
         end do
      end do
