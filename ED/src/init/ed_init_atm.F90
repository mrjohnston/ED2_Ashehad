         ...
         siteloop2: do isi = 1,cpoly%nsites
            ...
            patchloop2: do ipa = 1,csite%npatches
               ...
               if (csite%soil_tempk(1,ipa) == -100.0 .or. isoilstateinit == 0              &
                  ...
                  !----- For a lake site, soil_energy is computed differently. ------------! 
                  if (cpoly%islakesite(isi) == 1)  then
                      do k = 1, nzg
                         csite%soil_tempk(k,ipa) = csite%can_temp(ipa)
                
                         if (csite%soil_tempk(k,ipa) > t3ple) then
                             csite%soil_fracliq(k,ipa) = 1.0
                             csite%soil_water(k,ipa)   = 1.0
                             csite%soil_energy(k,ipa)  = cliqvlme                          &
                                                       * (csite%soil_tempk(k,ipa)          &
                                                       - tsupercool_liq) 
                         else
                             csite%soil_fracliq(k,ipa) = 0.0
                             csite%soil_water(k,ipa)   = 1.0
                             csite%soil_energy(k,ipa)  = cicevlme                          &
                                                       * csite%soil_tempk(k,ipa)
                         end if
                      end do
